...
EM_JS(JsRef, hiwire_incref_js, (JsRef idval), {
  if (idval & 1) {
...
// clang-format off
EM_JS_REF(JsRef, hiwire_int_js, (int val), {
  return Hiwire.new_value(val);
});

JsRef
hiwire_int(int val) {
  return hiwire_int_js(val);
}
// clang-format on

// clang-format off
EM_JS_REF(JsRef,
hiwire_int_from_digits_js, (const unsigned int* digits, size_t ndigits), {
  let result = BigInt(0);
...
EM_JS_REF(JsRef, hiwire_double_js, (double val), {
  return Hiwire.new_value(val);
});
...
EM_JS_REF(JsRef, hiwire_to_string_js, (JsRef idobj), {
...
EM_JS(int, JsArray_Push_unchecked_js, (JsRef idarr, JsRef idval), {
...
// clang-format off
EM_JS_REF(JsRef, JsObject_New_js, (), {
  return Hiwire.new_value({});
});

JsRef
JsObject_New(void) {
  return JsObject_New_js();
}
